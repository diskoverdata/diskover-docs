...
| 5 | Unfortunately, Zendesk doesn't offer a copy icon for codes like we have in our GitHub docs, but users can click once in a code block which will select all the text automatically, then they can just copy the whole code content.<ul><li>You should be able to click the **code block** formatting option and that should create a code block.</li><li>If you can get this to work your way, open the source code format as per #4 and use the following:<br>**``<pre><code id="codeSnippet">first line of code``<br>``second line of code``<br>``third line of code</code></pre>``**<br><br>This is what the results will be:<br><pre><code id="codeSnippet">first line of code    second line of code    third line of code</code></pre></li></ul>  |
